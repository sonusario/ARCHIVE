...
def initPop(n, nov):
    p = []
    for i in range(n):
        verts = np.random.randint(0, 95, nov)
...
def mixParents(i, pA, pB, PM, muRate):
    ...
    for i in range(len(verts)):
        if not randrange(rng): verts[i] = randrange(95)
...
bestMem, pop = findLeader(pop, numOfVerts, message, muRate, percentRandPop, itrMax, printFreq)
