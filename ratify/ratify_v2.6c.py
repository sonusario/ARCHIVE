...
@ $ -> ( ) # %
=======
Need to fix input/match_list mis-match
'''
...
def look_for_match(vals, vLUT, condouts, moreFlag):
    ...
    while there_is_a_match is False and len(condouts) > 0:
        ...
        vCount = len(vals)
        if match_count > vCount and moreFlag: #mCode == "more"
...
def call(tokens, fname):
    ...
    while len(tokens) > 0 and counter < pCount:
        ...
        counter += 1
    mCode = "more"
    if len(tokens) == 0 and counter == pCount:
        mCode = "exact"
    elif len(tokens) == 0:
        mCode = "low"
    ...
    there_is_a_match, out_list = look_for_match(values, vLUT, condouts, moreFlg)
