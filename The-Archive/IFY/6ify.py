...
def fn_define(defined,stack):
    if stack == []:
        return Error("Expected function specification LIST! Found NOTHING.")

    fokens = stack.pop(-1)
    if type(fokens) != type([]):
        got = fokens if type(fokens) == type("str") else "<anon_func>"
        return Error("Expected function specification LIST! Got " + got)
...
def replace_params(function, vLUT, output_spec):
    ...
    for item in output_spec:
        if type(item) == type([]):
            ...
            if type(val) == type([]):
                for v in val:
                    replacement_list.append(v)
            else:
                replacement_list.append(vLUT[item])
        elif item == '~':
            replacement_list.append(function)
        else:
...
def evaluate(defined, stack, tokens):
    ...
    while len(tokens) > 0 and not errFlag:
        token = tokens.pop(0)
        #print(format_stack(["IFY>"] + stack + tokens))
        if token == '@':
            print(format_stack(["IFY>"] + stack + [token] + tokens)) #debugger ;)
            prepend_tokens,errFlag = fn_call(defined,stack)
            if errFlag: continue
            pender(prepend_tokens,tokens,"prepend")
        elif token == '$':
            errFlag = fn_define(defined,stack)
            if errFlag: continue
        else:
            stack.append(token)
