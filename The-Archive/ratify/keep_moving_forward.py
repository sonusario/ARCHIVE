...
def define(tokens):
    ...
    while len(token) > 0:
        ...
        if type(sub_token) == type([]) or sub_token in ['$','@','/@','->']:
            raise SyntaxError("Error! Expected VAR. Found '" + sub_token + "'.")
        else:
            var_names.append(sub_token)

    # ---Get function name---
    if len(tokens) == 0:
        raise SyntaxError("Error! Expected FUNCTION NAME. Found NOTHING.")
    ...
    if len(tokens) == 0:
        raise SyntaxError("Error! Expected CONDITION_AND_OUTPUT LIST(s). " +
                          "Found NOTHING.")
    ...
    while len(tokens) > 0:
        ...
        if type(token) != type([]):
            raise SyntaxError("Error! Expected CONDITION_AND_OUPUT LIST. " +
                              "Found '" + token + "'.")
        elif token == [] or '->' not in token:
            raise SyntaxError("Error! Expected [OPTIONAL CONDITION(s) -> " +
                              "OPTIONAL OUTPUT(s)]. Found '" + token + "'.")
...
def read_from_tokens(tokens, recDepth=0):
    ...
    while len(tokens) > 0:
        ...
        if token == '(':
            ...
            STACK.append(read_from_tokens(token, recDepth + 1))
        elif token == ')':
            if recDepth > 0:
                return STACK
            else:
