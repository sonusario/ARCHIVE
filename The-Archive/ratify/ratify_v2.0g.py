...
def look_for_match(vals, vLUT, condout_lists, vCount):
    ...
    while there_is_a_match is False and len(condout_lists) > 0:
        ...
        match_list = read_from_tokens(match_list)
...
def anon_call(tokens, function_spec):
    fun_token = function_spec.pop(0)
    param_list = gen_param_list(fun_token,[])
    par_count = len(param_list)
    fun_token = function_spec.pop(0)
    condout_lists = get_condout_lists(function_spec, par_count)
...
def call(tokens, fname):
    ...
    if type(fname) == type([]):
        ...
        if type(token) != type([]):
            ...
            raise_NotEnoughData_error(len(token))
        elif token[1] == '->':
            return anon_call(tokens, token)
        else:
            define(token)
        return []
