...
def get_function_spec(function):
    ...
    function_spec = [params, fname]

    for condout in condouts:
        clst = []
        for c in condout["cond"]:
            clst.append(c)
        clst.append("->")
        for c in condout["out"]:
            clst.append(c)
            
        function_spec.append(clst)
...
def define(function):
    ...
    if type(function) != type([]):
        if function in defined:
            return False, True, get_function_spec(defined[function])
        else:
            raise SyntaxError("Ratify> Expected definition structured " +
                          "LIST or defined word. Found '" + str(function) + "'.")
    elif len(function) < 3:
        raise_NotEnoughData_error(len(function))
    elif function[1] == '->':
        return True, False, verified(function)
...
def FIRST(lst, cnt):
    ...
    elif len(lst) == 1 and len(lst[cnt:]) > 0:
        return lst[0:][0],False
    return '',False
...
def build_LUT(values, params, LUT):
    ...
    LP = len(p)
    while len(v) < LP:
        v.append('') ### MR.RUSTACEAN: this should be prepended. GIVEN '' and 'a' THEN 1:'' and 2: 'a'
    for i in range(LP):
...
def replace_params(val_LUT, param_filled_list):
    ...
    while len(pfL) > 0:
        ...
        if type(PF) == type([]):
            ...
            '''
            if vLUT[PF] == []:
                replacement_list.append('')# including this destroys cls
                continue
            '''
...
def call(tokens, fname):
    ...
    if type(fname) == type([]):
        ...
        if len(tokens) == 0:
            raise SyntaxError("Ratify> Expected definition structured " +
                              "LIST. Found NOTHING.")
        function_is_anon, function_is_defined, function = define(tokens.pop(-1))
        if not function_is_anon:
        ...
        if len(tokens) == 0:
            ...
            return [call(tokens, tokens.pop(-1))]
...
(((fn x y z B L)) LC
	(() -> )
	((fn) -> fn)
	((I x y z B L) -> x c_rp @ (y z B L) LC @)
	((K x y z B L) -> x c_rp @ (z B L) LC @)
	((S x y z B L) -> (x z) LC @ ((y z) LC @) @ (B L) LC @)
	(              -> FN: (fn) X: (x) Y: (y) Z: (z) body: (B) last: (L))) $ @
